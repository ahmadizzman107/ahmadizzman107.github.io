        ...
        <div class="col-md-8">
            <div class="card" style="box-shadow:-5px 2px;">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <div>
                        <form action={{ route('email-rating') }}
                            method="POST">
                            @csrf
                            @method('PUT')
                            @if (count($posts) > 0)
                                @foreach ($posts as $post)
                                    @if ($currentDate > $post->date && $currentTime > $post->time_end)
                                        <a href={{ route('show-rating', $post->id) }}><label
                                                style="display: block; font-size: 25px;">{{ $post->title }}</label></a>
                                        @if (count($post->rating) < 1 && count($post->participant) > 0)
                                            <input type="checkbox" class="broadcast_check" name="check[]"
                                                value={{ $post->id }}>
                                        @endif
                                        <small>Posted at {{ $post->created_at }}</small>
                                    @else
                                        <div><span style="font-size: 25px;">{{ $post->title }}</span>
                                            <span> - Event not finshed yet.</span>
                                        </div>
                                        <small>Posted at {{ $post->created_at }}</small>
                                    @endif
                                @endforeach
